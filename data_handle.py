        ...
        for ik, iv in v.items():
            if ik + '/kpi' not in kpi_json[k]:
                ...
                if len(iv) == 2:
                    ...
                    kpi_json[k][ik + '/kpi']['diff_percent'] = round(((iv[1]['avg'] - iv[0]['avg']) / (iv[0]['avg'] + 0.000000001)) * 100, 2)
                else:
                    if 'percent' in iv[0]:
    ...
    for k, v in data.items():
        ...
        for ik, iv in v.items():
            ...
            if 'jvm' in metric_name:
                ...
                v_min = 9999999999999999999999
                for iiv in iv:
                    final_metrics_list[k]['jvm'][metric_name]['timestamp'].append(iiv['timestamp'])
                    ...
                    final_metrics_list[k]['jvm'][metric_name]['avg'].append(value)

                    if 'percent' in iiv:
                        final_metrics_list[k]['jvm'][metric_name]['percent'].append(iiv['percent'])
                        if iiv['percent'] > v_max:
                            v_max = iiv['percent']
                        if iiv['percent'] < v_min:
                            v_min = iiv['percent']
                ...
                final_metrics_list[k]['jvm'][metric_name]['max'] = v_max
                final_metrics_list[k]['jvm'][metric_name]['min'] = v_min
            else:
                if metric_name not in final_metrics_list[k]:
                    final_metrics_list[k][metric_name] = {}
                    final_metrics_list[k][metric_name]['timestamp'] = []
